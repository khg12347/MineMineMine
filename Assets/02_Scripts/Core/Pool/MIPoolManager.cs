...
namespace MI.Core.Pool
{
    ...
    {
        [Title("풀 공통 설정")]
        [InfoBox("초기 생성 수 = 스테이지 폭 × 여유 행 수 기준으로 설정 권장 (기본: 8열 × 24행 = 192)")]
        [PropertyRange(8, 256)]
        [SerializeField] private int _initialPoolSize = 192;
            ...
            where T : Component
        {
            ...
            if (!_pools.TryGetValue(prefabId, out var poolObj))
                poolObj = CreatePool<T>(prefab, prefabId);
        ...
        private object CreatePool<T>(GameObject prefab, int prefabId) where T : Component
        {
            ...
            var pool = new MIObjectPool<T>(prefabComp, poolParent, _initialPoolSize, _autoExpand, _maxPoolSize);
